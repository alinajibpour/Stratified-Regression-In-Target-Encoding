...
from sklearn.neural_network import MLPRegressor  # Added for neural network
...
#encoded_df_Inpatient.to_csv(r"C:\Users\ra59xaf\Desktop\Thesis\encoded_df_Inpatient.csv", index=False)
#%%
...
#%%
# Nested Cross-Validation with Neural Network
def nested_cv_nn_with_existing_folds(X, y, outer_cv, inner_splits=5):
    outer_scores = []
    best_params = []
...
nested_scores, nested_params = nested_cv_nn_with_existing_folds(X, y, skf)
...
def plot_cv_comparison(fold_results, nested_scores):
    """
    Create comprehensive visualization comparing stratified and nested CV results for Neural Network
    """
    ...
    ax1.set_title('Neural Network: RMSE Comparison Across Folds', fontsize=12, pad=15)
    ...
    ax2.set_title('Neural Network: Distribution of RMSE Values', fontsize=12, pad=15)
    ...
    ax3.set_title('Neural Network: RMSE Trends Across Folds', fontsize=12, pad=15)
    ...
    ax4.set_title('Neural Network: Average RMSE with Standard Deviation', 
                  fontsize=12, pad=15)
    ax4.grid(True, linestyle='--', alpha=0.3)
    
    stats_text = (
        f'Neural Network Results\n\n'
        f'Stratified CV:\n'
