...
def process_fold(train_idx, test_idx, df, target, numerical_features, categorical_features):
    ...
    ('regressor', TransformedTargetRegressor(
        ...
        transformer=StandardScaler()  # or FunctionTransformer(np.log1p, np.expm1) if target is skewed
    ))
])
    ...
    'regressor__regressor__batch_size': [32,46]
}
...
if __name__ == "__main__":
    ...
    target = 'target_transformed' # Use transformed target for regression
